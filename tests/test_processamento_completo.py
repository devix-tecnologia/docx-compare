...
def processar_uma_versao(versao_data):
    ...
    try:
        ...
        if (
            is_first_version
            or request_data.get("versao_comparacao_tipo") == "modelo_template"
        ):
        ...
        # Executar comparação
        cmd = ["python", cli_path, original_path, modified_path, result_path]
        ...
        if result.returncode != 0:
            raise Exception(
                f"Erro na comparação (code {result.returncode}): {result.stderr}"
            )
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if versoes:
                versao = versoes[0]
                print(
                    f"📄 Testando com versão: {versao.get('versao', 'N/A')} ({versao.get('id', 'N/A')[:8]}...)"
                )
                print()
