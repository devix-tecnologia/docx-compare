...
def get_versao_complete_data(versao_id):
    ...
    try:
        ...
        print(f"✅ Dados da versão obtidos")
        return versao_data
...
def determine_original_file_id(versao_data):
    ...
    try:
        ...
        outras_versoes = [
            v for v in versoes 
            if v['id'] != current_versao_id and v.get('arquivo')
        ]
        ...
        print(f"📄 Usando versão mais recente: {original_file_id} (data: {versao_mais_recente['date_created']})")
    ...
    try:
        ...
        for mod in modifications:
            ...
            if response.status_code == 200:
                ...
                print(f"❌ Erro ao salvar modificação {mod['sort']}: {response.status_code}")
...
def update_versao_status(versao_id, result_url, total_modifications):
    ...
    try:
        ...
        if response.status_code == 200:
            print(f"✅ Versão atualizada com status 'concluido'")
...
def compare_versao():
    ...
    try:
        ...
        if not data or 'versao_id' not in data:
            return jsonify({
                'success': False,
                'error': 'Campo obrigatório: versao_id'
            }), 400
        ...
        if not modified_file_id:
            return jsonify({
                'success': False,
                'error': 'Versão não possui arquivo anexado'
            }), 400
