...
print(f"✅ Cliente HTTP inicializado")
...
def buscar_versoes_para_processar():
    ...
    try:
        ...
        print(f"🔍 Resultado RAW da query simples:")
        ...
        if simple_response.status_code == 200:
            ...
            versoes_response = requests.get(url_filtered, headers=DIRECTUS_HEADERS, params=params)
            
            print(f"🔍 Resultado RAW da query com filtro:")
...
def html_to_text(html_content):
    ...
    html_content = re.sub(r'<li[^>]*><p[^>]*>(.*?)</p></li>', r'• \1', html_content, flags=re.DOTALL)
...
def update_versao_status(versao_id, status, result_url=None, total_modifications=0, error_message=None, modifications=None):
    ...
    try:
        ...
        if modifications and status == "concluido":
            ...
            print(f"✅ {len(modifications_data)} modificações preparadas para salvar em uma única transação")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if modifications and status == "concluido":
                    print(f"✅ Versão atualizada com status '{status}' e {len(modifications)} modificações salvas em uma única transação")
                else:
                    print(f"✅ Versão atualizada com status '{status}'")
                return updated_versao
            else:
                print(f"❌ Erro ao atualizar versão: HTTP {response.status_code} - {response.text}")
                return None
        except Exception as e:
...
def processar_versao(versao_data):
    ...
    try:
        ...
        try:
            ...
            original_html_temp = tempfile.NamedTemporaryFile(mode='w', suffix='.html', delete=False)
            modified_html_temp = tempfile.NamedTemporaryFile(mode='w', suffix='.html', delete=False)
            
            # Converter usando pandoc
            subprocess.run(['pandoc', original_path, '-o', original_html_temp.name], check=True)
            subprocess.run(['pandoc', modified_path, '-o', modified_html_temp.name], check=True)
            
            # Ler e processar HTML
            with open(original_html_temp.name, 'r', encoding='utf-8') as f:
                original_html = f.read()
            with open(modified_html_temp.name, 'r', encoding='utf-8') as f:
            ...
            update_versao_status(versao_id, "concluido", result_url, len(modifications), modifications=modifications)
            
            print(f"✅ Versão {versao_id} processada com sucesso! {len(modifications)} modificações encontradas")
...
if __name__ == '__main__':
    ...
    print(f"⏰ Verificação automática a cada 1 minuto")
    print(f"🔒 Monitoramento de sinais ativo (SIGINT, SIGTERM, SIGHUP)")
    print(f"")
    print(f"📋 Endpoints de monitoramento:")
    print(f"  • GET  /health - Verificação de saúde")
    print(f"  • GET  /status - Status do processador")
