...
def download_file_from_directus(file_id, filename_prefix="file"):
    ...
    try:
        ...
        if filesize > MAX_FILE_SIZE:
            raise ValueError(f"Arquivo muito grande: {filesize} bytes (máximo: {MAX_FILE_SIZE} bytes)")
...
def analyze_differences(original_text, modified_text):
    ...
    additions = sum(1 for line in changes if line.startswith('+') and not line.startswith('+++'))
    deletions = sum(1 for line in changes if line.startswith('-') and not line.startswith('---'))
...
def generate_comparison_html(original_html, modified_html, statistics):
    ...
    for line in diff_lines:
        if line.startswith('+++') or line.startswith('---'):
            ...
            html_content += f'<div class="diff-line diff-added">+ {html.escape(line[1:])}</div>'
        elif line.startswith('-'):
            html_content += f'<div class="diff-line diff-removed">- {html.escape(line[1:])}</div>'
        elif line.startswith('@@'):
            html_content += f'<div class="diff-line diff-context"><strong>{html.escape(line)}</strong></div>'
        else:
            html_content += f'<div class="diff-line diff-context">{html.escape(line)}</div>'
...
def compare():
    ...
    try:
        ...
        if not is_valid_uuid(original_id) or not is_valid_uuid(modified_id):
            return jsonify({
                'success': False,
                'error': 'IDs devem ser UUIDs v4 válidos'
            }), 400
