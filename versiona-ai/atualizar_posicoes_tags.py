    ...
    for tag in tags_banco:
        ...
        if response.status_code == 200:
            ...
            if atualizadas <= 5:  # Log apenas primeiras 5
                print(
                    f"  ✅ Tag {tag_nome}: {pos_antiga_inicio}→{pos_nova_inicio} ({delta_inicio:+d}), {pos_antiga_fim}→{pos_nova_fim} ({delta_fim:+d})"
                )
        else:
            erros += 1
            if erros <= 3:  # Log apenas primeiros 3 erros
                print(
                    f"  ❌ Erro ao atualizar tag {tag_nome}: HTTP {response.status_code}"
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--modelo-id", required=True, help="ID do modelo de contrato")
    parser.add_argument(
        "--directus-url",
