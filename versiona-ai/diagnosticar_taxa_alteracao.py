...
def analyze_filtered_trivials(before: list[dict], after: list[dict]):
    ...
    if removed:
        print(f"\n{'='*80}")
        print(f"🗑️ MODIFICAÇÕES FILTRADAS (triviais < 10 chars)")
        print(f"{'='*80}")
...
def analyze_grouping_behavior(before_group: list[dict], after_group: dict):
    ...
    elif "INSERCAO" in tipos_antes and "ALTERACAO" not in tipos_antes and tipo_depois != "INSERCAO":
...
def main():
    ...
    taxa_baseline = stats_baseline["tipos"].get("ALTERACAO", 0) / stats_baseline["total"] * 100 if stats_baseline["total"] > 0 else 0
    taxa_filtered = stats_filtered["tipos"].get("ALTERACAO", 0) / stats_filtered["total"] * 100 if stats_filtered["total"] > 0 else 0
    taxa_grouped = stats_grouped["tipos"].get("ALTERACAO", 0) / stats_grouped["total"] * 100 if stats_grouped["total"] > 0 else 0
    
    print(f"\n1. Baseline:             {taxa_baseline:.1f}% ALTERACAO ({stats_baseline['tipos'].get('ALTERACAO', 0)}/{stats_baseline['total']})")
    print(f"2. Após filtro triviais: {taxa_filtered:.1f}% ALTERACAO ({stats_filtered['tipos'].get('ALTERACAO', 0)}/{stats_filtered['total']})")
    print(f"3. Após agrupamento:     {taxa_grouped:.1f}% ALTERACAO ({stats_grouped['tipos'].get('ALTERACAO', 0)}/{stats_grouped['total']})")
