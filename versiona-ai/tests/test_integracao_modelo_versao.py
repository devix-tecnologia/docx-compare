...
def test_fluxo_completo_modelo_versao(
    ...
    print("\n" + "="*70)
    print("ETAPA 1: Processando modelo (extraindo tags)")
    print("="*70)
    
    # Extrair tags do arquivo com tags
    ...
    print(f"\n✓ Tags extraídas: {len(tags_processadas)}")
    ...
    print(f"✓ Estrutura das tags validada")
    ...
    if tags_esperadas:
        ...
        print(f"✓ Quantidade de tags confere com expectativa")
    ...
    taxa_vinculacao = total_vinculadas / total_modificacoes if total_modificacoes > 0 else 0
    ...
    print(f"✓ Estrutura das modificações validada")
...
def test_coordenadas_alinhadas(
    ...
    if len(modificacoes) > 0:
        ...
        if len(tags_processadas) > 0:
            ...
            print(f"   Posições: {primeira_tag['posicao_inicio']}-{primeira_tag['posicao_fim']}")
        
        if len(modificacoes) > 0:
            primeira_mod = modificacoes[0]
            print(f"\n📝 Primeira modificação:")
            print(f"   Categoria: {primeira_mod['categoria']}")
            print(f"   Posições: {primeira_mod.get('posicao_inicio')}-{primeira_mod.get('posicao_fim')}")
            print(f"   Vinculada: {'Sim' if primeira_mod.get('clausula_id') else 'Não'}")
