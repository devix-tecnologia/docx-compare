...
class AlgoritmoRegex(AlgoritmoVinculacao):
    ...
    PADROES_ESTRUTURADOS: dict[str, Pattern] = {
        "monetario_br": re.compile(r"R\$\s*[\d.,]+", re.IGNORECASE),
        ...
        "contrato_id": re.compile(r"\b\d{4,}\b"),
    ...
    def calcular_posicoes(
        self, modificacoes: list[dict], texto_completo: str
    ) -> list[dict]:
        """
                ...
                resultado.append(
                    {
                        **mod,
                        "tag_vinculada": tag,
                        "_metodo_vinculacao": "fuzzy_fallback"
                        if tag
                        else "nao_vinculada",
                    }
                )
                ...
                resultado.append(
                    {
                        **mod,
                        ...
                        "_metodo_vinculacao": (
                            "regex_estrutura"
                            if tag and padrao_tipo != "literal"
                            else "fuzzy_fallback"
                            if tag
                            else "nao_vinculada"
                        ),
    ...
    def _detectar_tipo_padrao(self, texto: str) -> str | None:
        """
    ...
    def _buscar_com_regex(
        self, texto_busca: str, texto_completo: str, tipo_padrao: str
    ) -> tuple[int, int] | None:
        """
    ...
    def _comparar_datas(self, v1: str, v2: str) -> bool:
        ...
        if len(partes1) == 3 and len(partes2) == 3:
            ...
            if len(partes1[2]) == 2:
                partes1[2] = (
                    "20" + partes1[2] if int(partes1[2]) < 50 else "19" + partes1[2]
                )
            if len(partes2[2]) == 2:
                partes2[2] = (
                    "20" + partes2[2] if int(partes2[2]) < 50 else "19" + partes2[2]
                )
    ...
    def _buscar_tag_por_estrutura(
        self, mod: dict, tags: list[dict], tipo_padrao: str
    ) -> dict | None:
        """
    ...
    def _fallback_fuzzy(self, mod: dict, tags: list[dict]) -> dict | None:
        """
