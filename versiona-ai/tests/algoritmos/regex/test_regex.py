...
class TestComparacaoValores:
    ...
    def test_valores_monetarios_equivalentes(self):
        alg = AlgoritmoRegex()
        assert alg._valores_equivalentes("R$ 10.000,00", "R$ 10000,00", "monetario_br")
        assert alg._valores_equivalentes("R$1.500,50", "R$ 1500,50", "monetario_br")
    ...
    def test_datas_equivalentes(self):
        alg = AlgoritmoRegex()
        assert alg._valores_equivalentes("31/12/2024", "31-12-2024", "data_br")
        assert alg._valores_equivalentes("01/01/24", "01/01/2024", "data_br")
        assert not alg._valores_equivalentes("31/12/2024", "01/01/2024", "data_br")

    def test_percentuais_equivalentes(self):
        alg = AlgoritmoRegex()
        assert alg._valores_equivalentes("5%", "5 %", "percentual")
        assert alg._valores_equivalentes("10,5%", "10.5%", "percentual")
        assert not alg._valores_equivalentes("5%", "7%", "percentual")

    def test_numeros_equivalentes(self):
        alg = AlgoritmoRegex()
        assert alg._valores_equivalentes("30 dias", "30 dia", "prazo_dias")
...
class TestCalcularPosicoes:
    ...
    def test_calcular_posicoes_monetario(self):
        ...
        modificacoes = [
            {
                "tipo": "ALTERACAO",
                "conteudo": {"original": "R$ 10.000,00", "novo": "R$ 15.000,00"},
            }
        ]
    ...
    def test_calcular_posicoes_nao_encontrado(self):
        ...
        modificacoes = [
            {
                "tipo": "ALTERACAO",
                "conteudo": {"original": "R$ 99.999,99", "novo": "R$ 111.111,11"},
            }
        ]
