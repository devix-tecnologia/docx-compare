...
# Configuração
DIRECTUS_LOCAL_URL = "http://localhost:8065"  # Porta do Directus no docker-compose.ui-test.yml
...
def baixar_versao_directus(versao_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if versao and versao.get("id"):
            ...
            print(f"   - Modificações: {len(mods) if isinstance(mods, list) else 'N/A'}")
            print(f"   - Tags do modelo: {len(tags) if isinstance(tags, list) else 'N/A'}")
...
def extrair_dados_para_algoritmo(versao: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for tag in tags_raw:
        ...
        tags.append({
            "id": tag.get("id"),
            "titulo": tag.get("tag_nome") or tag.get("titulo") or f"Tag {tag.get('id')}",
            "texto": tag.get("conteudo") or tag.get("texto_tag") or tag.get("texto", ""),
    ...
    for mod in modificacoes_raw:
        ...
        modificacoes.append({
            ...
            "clausula_vinculada_esperada": mod.get("clausula"),  # Ground truth do Directus
        })
    ...
    if hasattr(algoritmo, "obter_estatisticas"):
        ...
        for estrategia, dados_est in stats.items():
            if dados_est["count"] > 0:
                print(f"   - {estrategia}: {dados_est['count']} ({dados_est['percentage']:.1f}%)")
...
def comparar_algoritmos(dados: Dict[str, Any], algoritmos_nomes: list = None):
    ...
    for nome in algoritmos_nomes:
        if nome not in ALGORITMOS:
            print(f"⚠️  Algoritmo '{nome}' não encontrado. Disponíveis: {', '.join(ALGORITMOS.keys())}")
            continue
    ...
    resultados_ordenados = sorted(
        resultados, 
        key=lambda r: r["taxa_vinculacao"], 
        reverse=True
    )
    ...
    for i, res in enumerate(resultados_ordenados):
        ...
        print(f"{medalha} {i+1}. {res['algoritmo']}: {res['taxa_vinculacao']:.1f}% "
...
def main():
    ...
    parser.add_argument(
        "--versao", "-v",
        default=VERSAO_ID,
        help=f"ID da versão (default: {VERSAO_ID})"
    )
    parser.add_argument(
        "--verbose",
        action="store_true",
        help="Imprimir detalhes de cada modificação"
    )
    parser.add_argument(
        "--salvar",
