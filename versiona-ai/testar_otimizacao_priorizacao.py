...
def test_insercao_remocao_vira_alteracao():
    ...
    assert result[0].get("tipo") == "ALTERACAO", (
        f"Esperava ALTERACAO, got {result[0].get('tipo')}"
    )
...
def test_apenas_insercao_continua_insercao():
    ...
    print(f"{'=' * 60}")
    print("Antes: 2 INSERCAOes")
    print(
        f"Depois: {len(result)} modificação, tipo: {result[0].get('tipo') if result else 'N/A'}"
    )

    assert len(result) == 1
    assert result[0].get("tipo") == "INSERCAO"

    print("✅ PASSOU: Grupo puro de INSERCAO continua INSERCAO!\n")
...
def test_alteracao_com_outros_continua_alteracao():
    ...
    print(f"{'=' * 60}")
    print("Antes: 3 modificações (1 ALTERACAO, 1 INSERCAO, 1 REMOCAO)")
    print(
        f"Depois: {len(result)} modificação, tipo: {result[0].get('tipo') if result else 'N/A'}"
    )
    ...
    print("✅ PASSOU: Grupo com ALTERACAO continua ALTERACAO!\n")
