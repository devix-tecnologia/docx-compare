...
def main():
    ...
    print(f"\n📄 Versão: {versao_id}")
    print("⏱️ Processando... (pode levar ~2 minutos)")
    ...
    print(
        f"{'Baseline (Task-016)':<30} {stats_sem['total']:>8} {stats_sem['tipos'].get('ALTERACAO', 0):>12} {stats_sem['taxa_alteracao']:>7.1f}%  {'-':>10}"
    )

    reducao_com = (
        ((stats_sem["total"] - stats_com["total"]) / stats_sem["total"] * 100)
        if stats_sem["total"] > 0
        else 0
    )
    print(
        f"{'Semântico padrão':<30} {stats_com['total']:>8} {stats_com['tipos'].get('ALTERACAO', 0):>12} {stats_com['taxa_alteracao']:>7.1f}%  {reducao_com:>9.1f}%"
    )

    reducao_alt = (
        ((stats_sem["total"] - stats_alt["total"]) / stats_sem["total"] * 100)
        if stats_sem["total"] > 0
        else 0
    )
    print(
        f"{'Semântico (same_type=True)':<30} {stats_alt['total']:>8} {stats_alt['tipos'].get('ALTERACAO', 0):>12} {stats_alt['taxa_alteracao']:>7.1f}%  {reducao_alt:>9.1f}%"
    )
    ...
    print(
        f"  Padrão:        {delta_taxa_com:+.1f}pp ({stats_sem['taxa_alteracao']:.1f}% → {stats_com['taxa_alteracao']:.1f}%)"
    )
    print(
        f"  Same_type=True: {delta_taxa_alt:+.1f}pp ({stats_sem['taxa_alteracao']:.1f}% → {stats_alt['taxa_alteracao']:.1f}%)"
    )

    print("\nRedução no total:")
    print(
        f"  Padrão:         {reducao_com:.1f}% ({stats_sem['total']} → {stats_com['total']})"
    )
    print(
        f"  Same_type=True: {reducao_alt:.1f}% ({stats_sem['total']} → {stats_alt['total']})"
    )
    ...
    if delta_taxa_com > 5:
        ...
        print(
            f"   - Baseline já tem apenas {stats_sem['taxa_alteracao']:.1f}% ALTERACAO"
        )
        print("   - Problema pode estar na Task-016 (classificação de tipos)")
        print("\n   Próximos passos:")
        print("   1. Revisar thresholds de similaridade na Task-016")
        print("   2. Ajustar lógica de classificação INSERCAO/REMOCAO → ALTERACAO")
        print(
            "   3. Considerar usar análise semântica (embeddings) para classificar tipos"
        )
